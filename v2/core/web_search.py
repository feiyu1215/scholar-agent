...
# ============================================================
# Phase 57: Paper Detail Fetcher (Cross-Document Cognition)
# ============================================================

@dataclass
class PaperDetail:
    """Detailed information about a single paper, beyond search-level metadata."""
    ...
    tldr: Optional[str]  # Semantic Scholar's auto-generated TLDR
    citation_count: Optional[int]
    reference_count: Optional[int]
    influential_citation_count: Optional[int]
    fields_of_study: List[str]
    # Key references this paper cites (for understanding its intellectual lineage)
    key_references: List[Dict[str, str]]  # [{title, year, venue}]
    # Key papers that cite this one (for understanding its impact)
    key_citations: List[Dict[str, str]]  # [{title, year, venue}]
    source: str = "semantic_scholar_detail"
    error: Optional[str] = None
...
def fetch_paper_detail(
    ...
    Phase 57: This gives the Agent the ability to "open" a paper found via search
    and read its detailed methodology context — like a reviewer pulling a paper
    off the shelf to compare methods.
    
    Lookup priority:
        1. paper_id (Semantic Scholar ID, most reliable)
        2. doi (via Semantic Scholar DOI lookup)
        3. title (search + best match, least reliable)
    
    Returns detailed info including: full abstract, TLDR, key references,
    key citations, fields of study, influential citation count.
    
    API: Semantic Scholar Paper Detail endpoint
    https://api.semanticscholar.org/graph/v1/paper/{paper_id}
    """
    # Determine the lookup identifier
    lookup_id = None
    if paper_id:
        lookup_id = paper_id
    elif doi:
        ...
        # Search first, then use the best match's paper_id
        # Try with retry on rate limit
        response = search_semantic_scholar(title, limit=3)
        ...
        if response.results:
            ...
            if best and best.url:
                # Extract paper_id from URL: https://www.semanticscholar.org/paper/{id}
                parts = best.url.rstrip("/").split("/")
                if parts:
                    lookup_id = parts[-1]
        if not lookup_id:
            ...
            if response.error and ("429" in response.error or "rate" in response.error.lower()):
                ...
                err_msg = f"Could not find paper matching title: '{title}'"
            return PaperDetail(
                paper_id="", title=title or "", authors=[], year=None,
                venue=None, abstract=None, tldr=None, citation_count=None,
                reference_count=None, influential_citation_count=None,
                fields_of_study=[], key_references=[], key_citations=[],
                error=err_msg
            )
    else:
        return PaperDetail(
            paper_id="", title="", authors=[], year=None,
            venue=None, abstract=None, tldr=None, citation_count=None,
            reference_count=None, influential_citation_count=None,
            fields_of_study=[], key_references=[], key_citations=[],
            error="Must provide paper_id, doi, or title"
        )

    # Check cache
    cache_k = _cache_key(f"detail::{lookup_id}", "paper_detail")
    if cache_k in _search_cache:
        cached = _search_cache[cache_k]
        if cached.results:
            # Reconstruct from cached data (stored as single-result SearchResponse)
            r = cached.results[0]
            return PaperDetail(
                paper_id=lookup_id, title=r.title, authors=r.authors,
                year=r.year, venue=r.venue, abstract=r.abstract,
                tldr=None, citation_count=r.citation_count,
                reference_count=None, influential_citation_count=None,
                fields_of_study=[], key_references=[], key_citations=[],
                source="cached"
            )

    # Fetch from Semantic Scholar Paper Detail API
    _semantic_scholar_limiter.wait()

    fields = (
        "title,authors,year,venue,abstract,tldr,citationCount,"
        "referenceCount,influentialCitationCount,fieldsOfStudy,"
        "references.title,references.year,references.venue,references.citationCount,"
        "citations.title,citations.year,citations.venue,citations.citationCount"
    )
    url = f"https://api.semanticscholar.org/graph/v1/paper/{urllib.parse.quote(lookup_id, safe=':')}?fields={fields}"
    ...
    try:
        ...
        # Parse fields of study (API returns either strings or dicts with "category" key)
        raw_fields = data.get("fieldsOfStudy") or []
        fields_of_study = []
        for f in raw_fields:
            if isinstance(f, str):
        ...
        # Parse key references (top 10 by citation count)
        references_raw = data.get("references") or []
        references_sorted = sorted(
            [r for r in references_raw if r.get("title")],
            key=lambda x: x.get("citationCount") or 0,
            reverse=True
        )[:10]
        key_references = [
            {"title": r.get("title", ""), "year": str(r.get("year", "?")), "venue": r.get("venue", "?")}
            for r in references_sorted
        ]

        # Parse key citations (top 10 by citation count — most influential papers citing this one)
        citations_raw = data.get("citations") or []
        citations_sorted = sorted(
            [c for c in citations_raw if c.get("title")],
            key=lambda x: x.get("citationCount") or 0,
            reverse=True
        )[:10]
        key_citations = [
            {"title": c.get("title", ""), "year": str(c.get("year", "?")), "venue": c.get("venue", "?")}
            for c in citations_sorted
        ]

        detail = PaperDetail(
            paper_id=data.get("paperId", lookup_id),
        ...
        # Cache the result (store as SearchResponse for compatibility)
        _search_cache[cache_k] = SearchResponse(
            query=lookup_id,
            results=[SearchResult(
                title=detail.title, authors=detail.authors, year=detail.year,
                venue=detail.venue, doi=None, url=None, abstract=detail.abstract,
                citation_count=detail.citation_count, source="paper_detail"
            )],
            total_found=1,
            source="paper_detail",
        )
        ...
        _disk_cache_put(cache_k, _search_cache[cache_k])
        ...
        if e.code == 429:
            ...
            try:
                ...
                # If retry succeeds, parse normally (fall through to same logic)
                # Re-parse inline to avoid deep nesting
                authors = [a.get("name", "") for a in data.get("authors", [])]
                tldr_data = data.get("tldr")
                tldr = tldr_data.get("text") if tldr_data else None
                raw_flds = data.get("fieldsOfStudy") or []
                fields_of_study = [f if isinstance(f, str) else f.get("category", "") for f in raw_flds if (isinstance(f, str) or (isinstance(f, dict) and f.get("category")))]
                references_raw = data.get("references") or []
                references_sorted = sorted([r for r in references_raw if r.get("title")], key=lambda x: x.get("citationCount") or 0, reverse=True)[:10]
                key_references = [{"title": r.get("title", ""), "year": str(r.get("year", "?")), "venue": r.get("venue", "?")} for r in references_sorted]
                citations_raw = data.get("citations") or []
                citations_sorted = sorted([c for c in citations_raw if c.get("title")], key=lambda x: x.get("citationCount") or 0, reverse=True)[:10]
                key_citations = [{"title": c.get("title", ""), "year": str(c.get("year", "?")), "venue": c.get("venue", "?")} for c in citations_sorted]
                return PaperDetail(
                    paper_id=data.get("paperId", lookup_id), title=data.get("title", ""),
                    authors=authors, year=data.get("year"), venue=data.get("venue"),
                    abstract=data.get("abstract"), tldr=tldr,
                    citation_count=data.get("citationCount"), reference_count=data.get("referenceCount"),
                    influential_citation_count=data.get("influentialCitationCount"),
                    fields_of_study=fields_of_study, key_references=key_references, key_citations=key_citations,
                )
            except Exception:
                pass  # Retry also failed, fall through to error return
            return PaperDetail(
                paper_id=lookup_id, title=title or "", authors=[], year=None,
                venue=None, abstract=None, tldr=None, citation_count=None,
                reference_count=None, influential_citation_count=None,
                fields_of_study=[], key_references=[], key_citations=[],
                error="Rate limited — try again in a moment"
            )
        error_msg = f"Semantic Scholar Detail API HTTP {e.code}"
        if e.code == 404:
            error_msg = f"Paper not found: {lookup_id}"
        return PaperDetail(
            paper_id=lookup_id, title=title or "", authors=[], year=None,
            venue=None, abstract=None, tldr=None, citation_count=None,
            reference_count=None, influential_citation_count=None,
