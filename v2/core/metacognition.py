...
class CognitiveState:
    ...
    # === V3 Phase 3: Reference to HypothesisModule (Single Source of Truth) ===
    _hypothesis_module_ref: Any = field(default=None, repr=False)

    def set_hypothesis_module(self, module: "HypothesisModule") -> None:
        """Called by Harness after init to establish hypothesis SoT reference."""
    ...
    def format_for_context(self) -> str:
        ...
        # === V3: Hypothesis projected from HypothesisModule (SoT) if available ===
        if self._hypothesis_module_ref is not None:
            hyp_status = self._hypothesis_module_ref.format_status()
            if hyp_status:
                parts.append(hyp_status)
        # Fallback: use local hypotheses field (backward compat / when module not set)
        ...
        if "hypotheses" in reflection_output:
            for h_data in reflection_output["hypotheses"]:
                if isinstance(h_data, dict) and "claim" in h_data:
                    ...
                    # === V3 Phase 3: Forward to HypothesisModule (SoT) ===
                    if self._hypothesis_module_ref is not None:
                        try:
                            status_val = h_data.get("status", "active")
                            if status_val == "active" and existing is None:
                                # New hypothesis -> generate in module
                                self._hypothesis_module_ref.generate(
                                    statement=claim,
                                    source="reflection",
                                    turn=self.last_updated_turn,
                                )
                            elif status_val in ("confirmed", "refuted", "suspended"):
                                # Status change -> try to resolve in module
                                # Find matching hypothesis in module by statement text
                                for mh in self._hypothesis_module_ref.active_hypotheses:
                                    if mh.statement == claim:
                                        status_map = {
                                            "confirmed": "supported",
                                            "refuted": "refuted",
                                            "suspended": "suspended",
